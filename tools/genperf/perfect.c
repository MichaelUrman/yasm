...
static ub4
permute(ub4 x,     /* input, a value in some range */
...
static void
scrambleinit(ub4 *scramble, /* hash is a^scramble[tab[b]] */
 ...
 */
static int
inittab(bstuff *tabb,   /* output, list of keys with b for (a,b) */
...
static void
initnorm(key *keys,      /* list of all keys */
         ...
         gencode *final) /* output, code for the final hash */
{
    ...
    if (phash_log2(alen) + phash_log2(blen) > UB4BITS) {
        ub4 initlev = (salt * 0x9e3779b9) &
                      0xffffffff; /* the golden ratio; an arbitrary value */
        ...
        final->used = 4;
        sprintf(final->line[0], "  unsigned long i,state[CHECKSTATE],rsl;\n");
        sprintf(final->line[1],
                "  for (i=0; i<CHECKSTATE; ++i) state[i]=0x%lx;\n", initlev);
        sprintf(final->line[2], "  phash_checksum(key, len, state);\n");
        ...
        if (smax <= 1) {
            ...
            sprintf(final->line[1],
                    "  rsl = ((val>>%ld)^scramble[tab[val&0x%lx]]);\n",
                    UB4BITS - phash_log2(alen), blen - 1);
        }
    }
}
...
static void
initinl(key *keys,      /* list of all keys */
        ub4 alen,       /* (a,b) has a in 0..alen-1, a power of 2 */
        ub4 blen,       /* (a,b) has b in 0..blen-1, a power of 2 */
        ...
        gencode *final) /* generated code for final hash */
{
    ...
    if (smax <= 1) {
        ...
        sprintf(final->line[0],
                "  unsigned long rsl = ((val & 0x%lx) ^ tab[val >> %ld]);\n",
                amask, UB4BITS - blog);
    } else {
        sprintf(final->line[0],
                "  unsigned long rsl = ((val & 0x%lx) ^ scramble[tab[val >> "
                "%ld]]);\n",
                amask, UB4BITS - blog);
    }
}
 ...
 */
static ub4
initkey(key *keys,      /* list of all keys */
...
static void
duplicates(bstuff *tabb, /* array of lists of keys with the same b */
           ub4 blen,     /* length of tabb, a power of 2 */
           key *keys, hashform *form) /* user directives */
{
    ub4 i;
    key *key1;
    key *key2;
...
static int
apply(bstuff *tabb, hstuff *tabh, qstuff *tabq, ub4 blen, ub4 *scramble,
      ub4 tail,
    ...
    hashform *form) /* TRUE if we should do a minimal perfect hash */
{
    ...
    for (q = 0; q < tail; ++q) {
        ...
        for (i = 0; i < limit; ++i) {
            ...
            tabq[tail].newval_q =
                (ub2)i; /* how to make parent (myb) use this hash */
...
static int
perfect(bstuff *tabb, hstuff *tabh, qstuff *tabq, ub4 blen, ub4 smax,
        ub4 *scramble, ub4 nkeys, hashform *form)
{
    ...
    for (j = maxkeys; j > 0; --j)
        for (i = 0; i < blen; ++i)
            if (tabb[i].listlen_b == j)
                if (!augment(tabb, tabh, tabq, blen, scramble, smax, &tabb[i],
                             nkeys, i + 1, form)) {
                    fprintf(stderr,
 ...
 */
static void
hash_ab(bstuff **tabb,  /* output, tab[] of the perfect hash, length *blen */
        ...
        hashform *form) /* user directives */
{
    ...
    *blen = 1;
    for (mykey = keys; mykey != (key *)0; mykey = mykey->next_k) {
        while (*alen <= mykey->a_k)
            *alen *= 2;
        while (*blen <= mykey->b_k)
            *blen *= 2;
    }
    if (*alen > 2 * *smax) {
        fprintf(
        ...
        fprintf(stderr, "  the smallest power of two greater or equal to any "
                        "legal hash.\n");
        exit(EXIT_FAILURE);
    }
    ...
    *tabb = (bstuff *)yasm_xmalloc((size_t)(sizeof(bstuff) * (*blen)));
    ...
    tabh = (hstuff *)yasm_xmalloc(
        sizeof(hstuff) * (form->perfect == MINIMAL_HP ? nkeys : *smax));
    ...
    if (!perfect(*tabb, tabh, tabq, *blen, *smax, scramble, nkeys, form)) {
        if (form->perfect == MINIMAL_HP) {
            fprintf(
                stderr,
                "fatal error: Cannot find perfect hash for user (A,B) pairs\n");
            exit(EXIT_FAILURE);
        } else {
            /* try with 2*smax */
            free((void *)tabh);
            *smax = *smax * 2;
            ...
            if (!perfect(*tabb, tabh, tabq, *blen, *smax, scramble, nkeys,
                         form)) {
                fprintf(stderr, "fatal error: Cannot find perfect hash for "
                                "user (A,B) pairs\n");
                exit(EXIT_FAILURE);
            }
        }
    ...
    *salt = 1;
    ...
    if (!used_tab) {
        ...
        sprintf(final->line[0],
                "  unsigned long rsl = (a ^ scramble[tab[b]]);\n");
    }
...
static void
initalen(ub4 *alen, /* output, initial alen */
         ...
         hashform *form) /* user directives */
{
     ...
     * 0..blen-1 are produced by applying a bitmask to the initial hash
     * function.
     *
        ...
        switch (phash_log2(*smax)) {
            ...
            case 18:
                if (form->speed == FAST_HS) {
                    ...
                    *blen =
                        (nkeys <= *smax * (5.0 / 8.0)) ? *smax / 4 : *smax / 2;
                }
                break;
            case 19:
            case 20:
...
*/
void
findhash(bstuff **tabb,  /* output, tab[] of the perfect hash, length *blen */
         ...
         hashform *form) /* user directives */
{
    ...
    if (form->hashtype == AB_HT) {
        hash_ab(tabb, alen, blen, salt, final, scramble, smax, keys, nkeys,
                form);
        return;
    }
    ...
    *salt = 0;
    ...
    for (trysalt = 1;; ++trysalt) {
        ...
        if (rslinit == 2) { /* initkey actually found a perfect hash, not just
                               distinct (a,b) */
            *salt = 1;
            *blen = 0;
            break;
            ...
            if (++bad_initkey >= RETRY_INITKEY) {
                /* Try to put more bits in (A,B) to make distinct (A,B) more
                 * likely */
                if (*alen < maxalen) {
                    *alen *= 2;
                } else if (*blen < *smax) {
                    *blen *= 2;
                    free(tabq);
                    free(*tabb);
                    *tabb = (bstuff *)yasm_xmalloc(
                        (size_t)(sizeof(bstuff) * (*blen)));
                    tabq = (qstuff *)yasm_xmalloc(
                        (size_t)(sizeof(qstuff) * (*blen + 1)));
                } else {
                    duplicates(*tabb, *blen, keys,
                               form); /* check for duplicates */
                    fprintf(stderr, "fatal error: Cannot perfect hash: cannot "
                                    "find distinct (A,B)\n");
                    exit(EXIT_FAILURE);
                }
                bad_initkey = 0;
